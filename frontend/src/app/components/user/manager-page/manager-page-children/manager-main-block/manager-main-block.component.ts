...
export class ManagerMainBlockComponent implements AfterViewInit {
	...
	public createProsumerList(prosumers): void {
		...
		for (const prosumer of prosumers) {
			if (this.prosumerList.nativeElement.children.length === 0) {
				this.createProsumerInfoEntry(prosumer.id, prosumer.house.hasBlackout);
			} else {
				let matchFound = false;
				for (const prosumerListElement of this.prosumerList.nativeElement.children) {
					if ('Prosumer ' + prosumer.id === prosumerListElement.textContent) {
						matchFound = true;
						const blackOutSvg = prosumerListElement.getElementsByClassName('online-status')[0];
						if (prosumer.house.hasBlackout) {
							if (blackOutSvg !== undefined) {
								blackOutSvg.classList.remove('online-status');
							}
						} else {
							if (blackOutSvg === undefined) {
								prosumerListElement.children[0].classList.add('online-status');
							}
						}
					}
				}
			...
			this.dialogService.confirmed().subscribe(confirmed => {
				if (confirmed) {
					this.deleteProsumerService.deleteProsumer(prosumerId).subscribe( () => {
						const item = document.getElementById('prosumer-info-item_' + prosumerId);
						if (item !== null) {
