...
class BuildingResolver extends BaseWithHistoryResolver {
	constructor(tableName: string) {
		super(tableName);
		this.queries.create = `
			INSERT INTO ${this.tableName}(battery_buffer, battery_limit, building_types_id, owner_id, histories_id)
		...
		this.queries.updateBatteryBuffer = `
			UPDATE ${this.tableName} SET battery_buffer = battery_buffer + $2 WHERE id = $1 RETURNING *`;
